...
description: "Create or refresh evidence-backed coding-agent knowledge for a repository or named functional module, including dedicated module knowledge bases and CODE-MAP-only updates. Do not use for explain-only walkthroughs or copy edits and translations that require no repository evidence."
---
...
- Treat existing knowledge as human-authored unless explicit provenance proves otherwise: generated frontmatter or a banner naming a generator, repository configuration or scripts declaring the output path, or a committed provenance file. Preserve supported guidance and rationale. A current-state claim describes what the repository does now; intent describes a goal, plan, rationale, or ownership claim not established by current implementation. Correct a contradicted current-state claim only when the requested scope and document contract select that claim. Selecting a root document for a module pointer or check does not select its repository-wide guidance, even when the module exposes a contradiction in that guidance; report such stale owner claims instead of editing them. Retain contradicted intent only as labelled unresolved intent.
...
1. **Scope and mode.** Identify the repository root, choose **Repository** or **Functional module** scope, extract the user's checkable requirements, and inventory existing knowledge before editing. For Functional module scope, read [module knowledge](./references/module-knowledge.md) and establish the module boundary before choosing documents or paths.
   - Inventory is complete only after searching the selected scope and its knowledge owners for `AGENTS.md`, `CLAUDE.md`, `MODULE.md`, `CONTEXT*.md`, `ARCHITECTURE.md`, `CODE-MAP.md`, ADR directories, and repository-local agent, rule, or instruction docs. Classify each candidate as selected, linked owner, unrelated, or stale before drafting.
...
2. **Build evidence.** Read [evidence rules](./references/evidence-rules.md) and [document contracts](./references/document-contracts.md). If this or any other referenced reference document cannot be read, report the missing reference and stop rather than proceeding with assumed rules. Inventory workspace declarations, manifests, build and deployment definitions, and declared entry points across the requested scope. Trace relevant entry points through behavior-bearing code and focused tests. Inspect repository-provided ways to start, drive, observe, and validate the system. For a functional module, inspect callers, registrations, dependencies, and shared owners outside the module far enough to prove its boundary without absorbing their facts into the module knowledge set.
   ...
   Evidence build is complete when every required ledger claim has a row, every high-risk current-state claim is verified, strongly inferred, or marked unknown, and every completeness or absence claim records a full-scope search. Give every documented lifecycle, mutation, migration, reconciliation, or repair flow its own runtime-boundary row; a shared transport row does not close its callers. Each row must name the actual trigger or caller, trace every effect through invocation, enqueue, dispatch, acknowledgement or fallback, and observable outcome, and account for replay ordering or concurrency and any test-double limitation. When a consumer indexes, enumerates, iterates, spreads, or serializes a persisted or decoded collection, compare its actual producer runtime type with the test fixture: an available repository-pinned hydrate, parse, or decode path requires a no-I/O probe through the actual consumer and serializer. Schema declarations, JSDoc, hand-built literals, and mocked tests alone do not close that row. Ordinary path links, headings, and formatting claims do not need ledger rows; verify them during mechanical checks.
...
3. **Select documents.** Apply the document contract value tests and ownership rules. A full functional module knowledge set always has `MODULE.md` after the sufficiency gate passes; a CODE-MAP-only request is the isolation exception and creates or refreshes only `CODE-MAP.md`. Create other documents only when their value tests pass. Split context glossaries by evidenced domain language, not by directory, service, deployment unit, technical layer, or the mere existence of a functional module.
...
4. **Draft or refresh.** Edit only selected, affected documents. Follow their contracts, preserve supported local guidance, and use canonical terminology consistently. Document stable relationships, constraints, source-of-truth locations, and observable feedback paths; report missing evidence or repository-local guardrails instead of guessing.
   ...
   In Refresh mode, remove or correct affected stale paths, commands, names, boundaries, tests, and generated-code mappings. Recreate a missing linked document only when its value test still passes. In Refresh and full regeneration, keep an existing functional module's knowledge location and stable key unless the user requests a move or repository configuration explicitly declares another location authoritative. For a CODE-MAP-only request, leave every other repository and module knowledge document byte-for-byte unchanged.

5. **Verify.**
   - Treat documented commands, source-of-truth or generated-output classifications, gated behavior, functional-module identity and boundary, domain cardinality/lifecycle/ownership, runtime representation, completion, or external-effect ordering across persistence, parser, SDK, cache, queue, transport, or serialization boundaries, and completeness, absence, uniqueness, or isolation claims as high-risk.
   ...
   - Reconcile every lifecycle, mutation, migration, reconciliation, or repair ledger row against its final owner passage. Confirm that the passage retains the actual trigger or caller, each effect's completion layer, fallback, replay ordering or concurrency, test-double limitation, and observable outcome; a shared transport passage does not close an individual flow.
...
6. **Report.** In the requested output language, use this compact report shape and omit empty sections: `Scope`, `Mode`, `Changed Files`, `Requirements Applied`, `Decisive Evidence`, `Consolidations`, `Stale or Unresolved Claims`, `Unknowns or Guardrail Gaps`, `Checks`.
