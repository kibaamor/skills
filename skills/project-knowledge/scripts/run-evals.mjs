...
function triggerTask(test, index) {
  return {
    ...
    tags: ['trigger', test.should_trigger ? 'positive-trigger' : 'negative-trigger'],
...
async function writeSuite(directory, tasks) {
  ...
  await writeFile(path.join(directory, 'wazaEval.yaml'), JSON.stringify(suite, null, 2))
  await Promise.all(
    tasks.map((task) =>
      writeFile(path.join(tasksDir, `${task.id}.yaml`), JSON.stringify(task, null, 2))
    )
  )
}
...
async function main() {
  ...
  const temporaryDir = await mkdtemp(path.join(os.tmpdir(), 'project-knowledge-evals-'))
  ...
  try {
    ...
    await Promise.all([rm(resultFile, { force: true }), rm(junitFile, { force: true })])
    ...
    console.log(`JUnit: ${(await exists(junitFile)) ? junitFile : 'not produced'}`)
  } finally {
